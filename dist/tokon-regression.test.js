...
describe('tsl-1-marvel-tokon replay', () => {
    it('Lowry and BlackVegeta cannot be re-paired before LR7', () => {
        ...
        const report = (bracket, id, winner) => {
            ...
            for (const m of ms) {
                if (!m.winner && m.status === 'pending' && m.participant1 && m.participant2)
                    m.status = 'ready';
            }
            return { ...next, matches: ms };
        };
        for (const [mid, winner] of WINS)
            b = report(b, `t_${mid}`, winner);
        ...
        const isOut = (bracket, who) => bracket.matches.some(m => m.bracketType === 'losers' && m.winner && m.winner !== who &&
            (m.participant1 === who || m.participant2 === who));
        const explore = (bracket, depth) => {
            if (depth > 60)
                return;
            if (isOut(bracket, 'Lowry') || isOut(bracket, 'BlackVegeta'))
                return;
            seen++;
            const ready = bracket.matches.find(m => m.status === 'ready' && m.participant1 && m.participant2 && !m.winner);
            if (!ready)
                return;
        ...
        explore(b, 0);
        console.log(`  explored ${seen} states; deepest losers round reached: LR${Math.max(...[...reached].filter(r => r.startsWith('losers')).map(r => Number(r.match(/R(\d+)/)[1])))}`);
        ...
        const earliest = Math.min(...[...clashes].filter(c => c.startsWith('losers')).map(c => Number(c.match(/R(\d+)/)[1])));
        expect(earliest, `earliest losers rematch was LR${earliest}, want LR7+`).toBeGreaterThanOrEqual(7);
    }, 30000);
});
//# sourceMappingURL=tokon-regression.test.js.map