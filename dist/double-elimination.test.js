...
function assertValidDEStructure(b, n, grandFinalReset = true) {
...
describe('generateDoubleElimination — N=13 (canonical flow-byes case)', () => {
    ...
    it('L3 (consolidate with bye) routes correctly via skip-bye to L4', () => {
        ...
        const skippingToL4 = l2WinnersDestinations.filter(d => d.targetMatch?.bracketType === 'losers' && d.targetMatch.round === 4);
        ...
        const intoL3 = l2WinnersDestinations.filter(d => d.targetMatch?.bracketType === 'losers' && d.targetMatch.round === 3);
...
describe('reportDoubleElimMatchResult', () => {
    ...
    it('handles GF1 — WB-bracket player wins → tournament over (GF2 marked bye)', () => {
        ...
        bracket = {
            ...bracket,
            matches: bracket.matches.map(m => m.id === gf1.id
                ? {
                    ...m,
    ...
    it('simulates a full 13-player DE tournament where seed 1 wins everything', () => {
        ...
        while (true) {
            ...
            iterations++;
            if (iterations > 100)
                throw new Error('Tournament simulation stuck');
        }
