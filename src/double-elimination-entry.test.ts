...
const p = (n: number): Participant => ({ id: `p${n}`, seed: n, name: `P${n}` });
...
const losersRound = (b: ReturnType<typeof top24>, r: number) =>
	b.matches.filter((m) => m.bracketType === 'losers' && m.round === r)
...
describe('buildDoubleEliminationWithEntry', () => {
	...
	it('gives every match except the last a forward pointer', () => {
		const b = top24();
		const dangling = b.matches.filter(
			(m) => !m.nextMatchId && !(m.bracketType === 'grand_final' && m.round === 2)
		);
...
describe('buildDoubleEliminationWithEntry — structure across shapes', () => {
	...
	for (const [bracketSize, direct, expectedRounds] of shapes) {
		it(`${bracketSize}x${direct}: one elimination per player bar the champion`, () => {
			...
			for (const m of b.matches) {
				if (m.nextMatchId) incoming.set(`${m.nextMatchId}#${m.nextMatchSlot}`, 1);
				if (m.loserNextMatchId) incoming.set(`${m.loserNextMatchId}#${m.loserNextMatchSlot}`, 1);
			}
			for (const m of losersMatches) {
				for (const slot of [1, 2] as const) {
					const seated = slot === 1 ? m.participant1 : m.participant2;
					expect(
						Boolean(seated) || incoming.has(`${m.id}#${slot}`),
						`${m.id} slot ${slot} can never be filled`
					).toBe(true);
				}
			}
		});
	...
	it('applies the searched crossover permutation to each drop round', () => {
		...
		const drop = (round: number, position: number) =>
			b.matches.find(
				(m) => m.bracketType === 'winners' && m.round === round && m.position === position
			)!.loserNextMatchId;
