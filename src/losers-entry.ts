 ...
 */
export function planEntryShape(bracketSize: number, directEntrants: number): EntryShape {
	...
	if (directEntrants < wr1Losers) {
		throw new EntryShapeError(
			`need at least ${wr1Losers} direct entrants to merge against winners round 1, got ${directEntrants}`
		);
	}
	const ratio = directEntrants / wr1Losers;
	if (!Number.isInteger(ratio) || !isPowerOfTwo(ratio)) {
		throw new EntryShapeError(
			`${directEntrants} cannot reduce cleanly to ${wr1Losers}; ` +
	...
	const entryMatchesPerRound: number[] = [];
