...
export function generateDoubleElimination(
  ...
  // === Build WB routing (mirror of SE flow-byes) ===
  type WBSlotRef =
  ...
  const wbByeAtTop = (round: number): boolean => round % 2 === 1;

  const buildWBInputSlots = (round: number): WBSlotRef[] => {
    const matchesInRound = wbCounts[round - 1]!;
    const byeInRound = wbByes[round - 1]!;
    const realSlots: WBSlotRef[] = [];
  ...
  const buildWBOutputs = (round: number): WBOutputRef[] => {
    const matchesInRound = wbCounts[round - 1]!;
    const byeInRound = wbByes[round - 1]!;
    const matchOutputs: WBOutputRef[] = [];
    for (let p = 1; p <= matchesInRound; p++) {
  ...
  const wbOutputKey = (o: WBOutputRef): string =>
    o.kind === 'matchWinner' ? `wm_${o.round}_${o.position}` : `wb_${o.round}`;

  const wbOutputToSlot = new Map<string, WBSlotRef>();
  for (let r = 1; r < wbRounds; r++) {
    const outs = buildWBOutputs(r);
    const slots = buildWBInputSlots(r + 1);
    if (outs.length !== slots.length) {
      throw new Error(
        `WB linking mismatch: round ${r} outputs=${outs.length}, round ${r + 1} slots=${slots.length}`
      );
    }
  ...
  const resolveWBDest = (
    startOutput: WBOutputRef
  ): { matchId: string; slot: 1 | 2 } | null => {
    let cur: WBOutputRef = startOutput;
    while (true) {
  ...
  for (const m of matches) {
    if (m.bracketType !== 'winners') continue;
    if (m.round < wbRounds) {
  ...
  // === Build LB routing ===
  //
  // For each LB round R we maintain TWO input slot lists:
  //   - carryoverInputSlots(R): receives prev LB round's outputs (top-to-bottom).
  //   - dropInInputSlots(R):    receives WB round wbLoserFeedRound's losers
  //                             (top-to-bottom). Empty for consolidate rounds.
  //
  // A bye slot may live on either list:
  //   - Carryover bye when carryover > matches (one extra carryover player).
  //   - DropIn bye when dropIn > matches (one extra drop-in player).
  //   - In consolidate rounds, only carryover side; the bye (if any) is on it.
  //
  // Bye placement (top vs bottom) within its list follows the alternation rule:
  //   bye at top  if round % 2 === 1
  //   bye at bottom if round % 2 === 0
  //
  // Round R's OUTPUT ordering top-to-bottom = [bye?, M1.winner, M2.winner, ...]
  // with bye at top if R odd, bottom if R even. This output stream feeds the
  // carryover side of round R+1 in order.

  type LBSlotRef =
    | { kind: 'lbMatchSlot'; round: number; position: number; slot: 1 | 2 }
    | { kind: 'lbBye'; round: number; side: 'carry' | 'drop' };

  type LBOutputRef =
    | { kind: 'lbMatchWinner'; round: number; position: number }
    | { kind: 'lbBye'; round: number };

  const lbByeAtTop = (round: number): boolean => round % 2 === 1;

  const lbInfoByRound = new Map<number, typeof lbShape[number]>();
  for (const info of lbShape) {
    lbInfoByRound.set(info.round, info);
  }

  const buildLBCarryoverSlots = (round: number): LBSlotRef[] => {
    const info = lbInfoByRound.get(round);
    if (!info) return [];
    const carryMatchCount = Math.min(info.carryover, info.matches);
    const realSlots: LBSlotRef[] = [];
    if (info.isDropIn) {
    ...
    if (carryBye === 1) {
      const byeSlot: LBSlotRef = { kind: 'lbBye', round, side: 'carry' };
      return lbByeAtTop(round) ? [byeSlot, ...realSlots] : [...realSlots, byeSlot];
    }
    return realSlots;
  };

  const buildLBDropInSlots = (round: number): LBSlotRef[] => {
    const info = lbInfoByRound.get(round);
    if (!info || !info.isDropIn) return [];
    if (info.round === 1) {
      ...
      if (dropBye === 1) {
        const byeSlot: LBSlotRef = { kind: 'lbBye', round, side: 'drop' };
        return lbByeAtTop(round) ? [byeSlot, ...realSlots] : [...realSlots, byeSlot];
      }
      return realSlots;
    }
  ...
  const buildLBOutputs = (round: number): LBOutputRef[] => {
    const info = lbInfoByRound.get(round);
    if (!info) return [];
    const matchOutputs: LBOutputRef[] = [];
    for (let p = 1; p <= info.matches; p++) {
      matchOutputs.push({ kind: 'lbMatchWinner', round, position: p });
    }
    if (info.byes === 1) {
      const byeOut: LBOutputRef = { kind: 'lbBye', round };
      return lbByeAtTop(round) ? [byeOut, ...matchOutputs] : [...matchOutputs, byeOut];
    }
    return matchOutputs;
  };

  const lbOutputKey = (o: LBOutputRef): string =>
    o.kind === 'lbMatchWinner' ? `lm_${o.round}_${o.position}` : `lb_${o.round}`;

  // Map LB round R outputs → round R+1 carryover slots (top-to-bottom).
  const lbOutputToCarrySlot = new Map<string, LBSlotRef>();
  for (let r = 1; r < lbRounds; r++) {
    const outs = buildLBOutputs(r);
    const slots = buildLBCarryoverSlots(r + 1);
    if (outs.length !== slots.length) {
      throw new Error(
        `LB carryover linking mismatch: round ${r} outputs=${outs.length}, round ${r + 1} carryover slots=${slots.length}`
      );
    }
    for (let i = 0; i < outs.length; i++) {
  ...
  const resolveLBDest = (
    startOutput: LBOutputRef
  ): { matchId: string; slot: 1 | 2 } | null => {
    let cur: LBOutputRef = startOutput;
    while (true) {
      const slot = lbOutputToCarrySlot.get(lbOutputKey(cur));
      if (!slot) return null; // No further LB round — this is the LB final's output.
  ...
  for (const m of matches) {
    if (m.bracketType !== 'losers') continue;
  ...
  const wbRoundToLBDropIn = new Map<number, number>();
  for (const info of lbShape) {
  ...
  for (let wbR = 1; wbR <= wbRounds; wbR++) {
    ...
    if (dropInSlots.length !== wbMatchesInR) {
      throw new Error(
        `LB drop-in linking mismatch: WB R${wbR} losers=${wbMatchesInR}, LB R${lbR} dropIn slots=${dropInSlots.length}`
      );
    }
  ...
  if (r1Slots.length !== participantCount) {
    throw new Error(
      `Internal: WB R1 input slot count ${r1Slots.length} !== participantCount ${participantCount}`
    );
  }
  ...
  for (let i = 0; i < r1Slots.length; i++) {
    ...
    if (slot.kind === 'matchSlot') {
      const targetMatch = matches.find(
        m => m.bracketType === 'winners' && m.round === slot.round && m.position === slot.position
      );
...
export function reportDoubleElimMatchResult(
  ...
  const loserSeed =
    match.participant1 === winnerId ? match.participant2Seed : match.participant1Seed;
