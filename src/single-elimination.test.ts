...
function getR1Matches(b: Bracket): Match[] {
  return b.matches.filter(m => m.round === 1).sort((a, b) => a.position - b.position);
}

function assertValidStructure(b: Bracket, n: number): void {
  ...
  for (const start of b.matches) {
    ...
    while (cur && cur.nextMatchId) {
      ...
      cur = next;
      steps++;
      if (steps > b.matches.length) throw new Error('infinite loop in advancement graph');
    }
...
describe('generateSingleElimination — N=7 (one bye in R1 only)', () => {
  it('produces 6 matches', () => {
    ...
    const seed1InR2 = r2Matches.some(
      m => m.participant1Seed === 1 || m.participant2Seed === 1
    );
