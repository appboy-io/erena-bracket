...
describe('generateDoubleElimination', () => {
  ...
  it('sets up winners bracket seeding correctly', () => {
    ...
    const winnersR1 = bracket.matches.filter(
  ...
  it('links winners finals to grand finals', () => {
    ...
    const winnersFinals = bracket.matches.find(
      m => m.bracketType === 'winners' && m.round === 2
    )!;
    const grandFinals1 = bracket.matches.find(
...
describe('reportDoubleElimMatchResult', () => {
  it('advances winner in winners bracket', () => {
    ...
    const winnersR2 = updated.matches.find(
      m => m.bracketType === 'winners' && m.round === 2
    )!;
  ...
  it('sends loser to losers bracket', () => {
    ...
    const hasPlayer4 = losersR1.some(
      m => m.participant1 === 'player_4' || m.participant2 === 'player_4'
    );
  ...
  it('handles grand finals - winners bracket player wins', () => {
    ...
    const gf1 = bracket.matches.find(
      m => m.bracketType === 'grand_final' && m.round === 1
    )!;
    ...
    const gf2 = updated.matches.find(
      m => m.bracketType === 'grand_final' && m.round === 2
    )!;
  ...
  it('handles grand finals reset - losers bracket player wins GF1', () => {
    ...
    const gf1 = initialBracket.matches.find(
      m => m.bracketType === 'grand_final' && m.round === 1
    )!;
...
function winnersUpstream(matches: Match[], wbId: string, acc = new Set<string>()): Set<string> {
	if (acc.has(wbId)) return acc;
	acc.add(wbId);
...
function winnersPlayedIn(
	...
	for (const f of feeders) {
		if (f.bracketType === 'winners') {
			for (const id of winnersUpstream(matches, f.id)) acc.add(id);
		} else {
			for (const s of [1, 2] as const) {
				for (const id of winnersPlayedIn(matches, f.id, s, memo)) acc.add(id);
			}
		}
	}
	return acc;
...
function losersRematchSlack(bracket: Bracket, bracketSize: number): number[] {
	...
	for (const m of ms.filter((x) => x.bracketType === 'losers').sort((a, b) => a.round - b.round)) {
		...
		for (const id of a1) {
			...
			if (!earliest.has(r) || m.round < earliest.get(r)!) earliest.set(r, m.round);
		}
	}
...
describe('losers crossover is optimal', () => {
	...
	it('tsl-1-marvel-tokon: a WR3 pair cannot be re-paired at LR6 in a 32 bracket', () => {
		...
		const wr3 = new Set(
			bracket.matches.filter((m) => m.bracketType === 'winners' && m.round === 3).map((m) => m.id)
		);
		const memo = new Map<string, Set<string>>();
		...
		for (const m of bracket.matches.filter((x) => x.bracketType === 'losers' && x.round <= 6)) {
			...
			if (clash.length) offenders.push(`${m.id} <- ${clash.join(',')}`);
		}
